...
<?php include('ignis/config/main.php');?>
<?php require_once "ignis/config/layout_$layout.php"; ?>
...
<html lang="en" dir="<?= $dir; ?>">
	<head>
        ...
        <link rel="stylesheet" type="text/css" href="ignis/compiled/css/animate.min.css">
					...
					<?php if(getSetting('installed', 'value2') == 1){ ?>
						...
						if ($show_header_links) {
							foreach ($header_links as $key => $value) {
								echo '<li><a href="'.$value['Link'].'"><i class="fa fa-'.$value['Icon'].'"></i>'.$value['Title'].'</a></li>';
							}
						}
						?>
					  ...
					  <ul class="nav navbar-nav navbar-right">
                          <?php if(prometheus::loggedin()) { ?>
                          <li class="dropdown">
                          <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false"><img src="<?php echo getUserSetting('steam_avatar', (isset($UID_a) ? $UID_a : null)); ?>" class="avatar" /></a>
                          <ul class="dropdown-menu" role="menu">
                              <?php } ?>
                              <?php if(prometheus::loggedin()) { ?>
                                  ...
                                  <?php } ?>
                                  <li><a href="logout.php?csrf_token=<?= csrf_token(); ?>"><i class="fa fa-sign-out"></i> <?= lang('sign_out'); ?></a></li>
                              <?php } else { ?>
                              <?php if(!prometheus::loggedin()) { ?>
...
<?php } ?>
			    ...
			    <style>
		    .c-page{width:100%;height:100vh;display:table}.c-page>div{display:table-cell;vertical-align:middle;text-align:center}.c-page.not-configured{background:#FF4136;color:#fff;width:100%;height:100vh}
	    </style>
	    <?php if($done == false): ?>
		    <div class="c-page not-configured">
			    <div>
				    <h1>Wait!</h1>
				    <h3>You are not done configuring your new addon!</h3>
					<h3>Please read and finish configuring in the <code>ignis/config</code> folder!</h3>
			    </div>
		    </div>
		    <?php die; endif; ?>